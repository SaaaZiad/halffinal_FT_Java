...
public class AvailableTicketsFacade {
  ...
  public void getProductsPage(Model model, String keywordDep, String keywordDest, String keywordDepTime,
    ...
    List<Ticket> depTickets = ticketService.findByDeparture(keywordDep);
    List<Ticket> destTickets = ticketService.findByDestination(keywordDest);
    List<Ticket> depTimeTickets = ticketService.findByDepTime(keywordDepTime);
    List<Ticket> transportTickets = ticketService.findByTransport(keywordTransport);
    List<Ticket> inter1Tickets = new ArrayList<Ticket>();
    List<Ticket> inter2Tickets = new ArrayList<Ticket>();
    List<Ticket> presortTickets = new ArrayList<Ticket>();
    List<Ticket> finalTickets = new ArrayList<Ticket>();
    for(Ticket t: depTickets) {
      if(destTickets.contains(t)) {
        inter1Tickets.add(t);
      }
    }
    for(Ticket t: inter1Tickets) {
      if(transportTickets.contains(t)) {
        inter2Tickets.add(t);
      }
    }
    for(Ticket t: inter2Tickets) {
      if(depTimeTickets.contains(t)) {
        presortTickets.add(t);
      }
    }
